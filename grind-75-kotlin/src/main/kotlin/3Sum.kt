...
fun threeSum(nums: IntArray): List<List<Int>> {
    ...
    for (first in 0 until nums.lastIndex - 1) {
        if (first > 0 && nums[first] == nums[first - 1]) continue

        var second = first + 1
        var third = nums.lastIndex
        val targetSum = 0 - nums[first]

        while (second < third) {
            val currentSum = nums[second] + nums[third]

            when {
                currentSum < targetSum -> second++
                currentSum > targetSum -> third--
                else -> {
